...
def train_tree(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               ...
               merge_optimizer, output_lang, num_pos, epoch, english=False):
    ...
    len_nums = []
    for jjj in num_pos:
        len_nums.append(len(jjj))
    '''
    training_pairs = []
    if False:
        for ii in range(len(input_length)):
            if True:
                encoder_optimizer.zero_grad()
                seq_encoder_optimizer.zero_grad()
                _, problem_output = encoder(input_var[:,ii].unsqueeze(-1), torch.tensor([input_length[ii]]))
                eq_output = seq_encoder.cal_grad(target[:,ii].unsqueeze(-1).cuda(), target_length[ii])
                #out = torch.bmm(problem_output.unsqueeze(1), eq_output.unsqueeze(-1))
                #out = f.cosine_similarity(problem_output, eq_output)
                out = torch.sigmoid(torch.bmm(problem_output.unsqueeze(1), eq_output.unsqueeze(-1)))
                #loss_score = f.binary_cross_entropy(out, torch.ones(out.size()).cuda())
                loss_score = out.squeeze()
                loss_score.backward()
                abs_grad = torch.norm(seq_encoder.embedded.grad.squeeze(), p = 2, dim = 1)

                abs_grad[target[:,ii] == 0] = 1e3
                m_indexs = torch.topk(abs_grad, min(3, input_length[ii]), largest=False)[1]
                manipulate_index = random.choice(m_indexs)
                #manipulate_index = torch.min(abs_grad, 0)[1]
            else:
                manipulate_index = random.randint(0, int(target_length[ii]) - 1)
            #m_indexs = torch.topk(abs_grad, min(3, abs_grad.size(0)), largest=False)[1]
            #manipulate_index = random.choice(m_indexs)
            ...
            manipulate_token = int(target[:,ii][manipulate_index])
            len_num = len_nums[ii]
            if len_num == 1:
                continue
            random_index = random.randint(0, len(input_length) - 1)
            cnt = 0
            break_flag = False
            while len_nums[random_index] > len_num or target_length[random_index] == target_length[ii]:
                cnt += 1
                if cnt >= 500:
                    break_flag = True
                    break

                if len_nums[random_index] <= len_num and target_length[random_index] == target_length[ii]:
                    if torch.sum(torch.abs(target[:target_length[random_index],random_index] - target[:target_length[ii],ii])) != 0:
                        break            
                random_index = random.randint(0, len(input_length) - 1)
            if break_flag:
                continue
            ...
            if False:
                ...
                seq_encoder_optimizer.zero_grad()
                _, problem_output = encoder(input_var[:,ii].unsqueeze(-1), torch.tensor([input_length[ii]]))
                eq_output = seq_encoder.cal_grad(target[:,random_index].unsqueeze(-1).cuda(), target_length[random_index])
                out = torch.sigmoid(torch.bmm(problem_output.unsqueeze(1), eq_output.unsqueeze(-1)))
                #out = f.cosine_similarity(problem_output, eq_output)
                loss_score = out.squeeze()
                #loss_score = f.binary_cross_entropy(out, torch.ones(out.size()).cuda())
                loss_score.backward()
                abs_grad = torch.norm(seq_encoder.embedded.grad.squeeze(), p = 2, dim = 1)
                abs_grad[target[:,random_index] == 0] = -1e3
                #random_manipulate_index = torch.max(abs_grad, 0)[1]
                m_indexs = torch.topk(abs_grad, min(3, target_length[random_index]), largest=True)[1]
                random_manipulate_index = random.choice(m_indexs)
            else:
                random_manipulate_index = random.randint(0, int(target_length[random_index]) - 1)

            #random_manipulate_index = random.randint(0, int(target_length[random_index]) - 1)
            random_manipulate_token = int(target[:,random_index][random_manipulate_index])

            training_pairs.append((input_var[:,ii], input_length[ii], target[:,ii], target_length[ii], 
                                    manipulate_index, manipulate_token, len_num, 
                                    target[:,random_index], target_length[random_index], random_manipulate_index, random_manipulate_token, len_nums[random_index]))
        ...
        for training_pair in training_pairs:
            max_target_len = max(training_pair[3], training_pair[8])
            total_cnt_grad += 1
            #target_ori = training_pair[2][:training_pair[3]].cuda().long()
            target_ori = torch.cat([training_pair[2][:training_pair[3]].cuda(), torch.zeros(max_target_len - training_pair[3]).cuda()]).long()
            label = [1]
            target_list = [target_ori]
            _,problem_output = encoder(training_pair[0].unsqueeze(-1).cuda(), torch.tensor([training_pair[1]]))

            if training_pair[5] < num_start:
                temp_list = copy.deepcopy(op_list)
            else:
                temp_list = [7 + i for i in range(training_pair[6])]
            if training_pair[5] != 4 and training_pair[5] != 5 and training_pair[5] != 6 and training_pair[5] != 22:
                temp_list.remove(int(training_pair[5]))
            try:
                for token in temp_list:
                    temp = copy.deepcopy(target_ori)
                    temp[training_pair[4]] = token
                    target_list.append(temp)
                    label.append(0)
            except:
                print(target_ori)
            len_part_1 = copy.deepcopy(len(target_list))
            target_neg = torch.cat([training_pair[7][:training_pair[8]].cuda(), torch.zeros(max_target_len - training_pair[8]).cuda()]).long()
            target_list.append(target_neg)
            label.append(0)


            if training_pair[10] < num_start:
                temp_list = copy.deepcopy(op_list)
            else:
                temp_list = [7 + i for i in range(training_pair[6])]
            if training_pair[10] != 5 and training_pair[10] != 6 and training_pair[10] != 4 and training_pair[10] != 22:
                temp_list.remove(int(training_pair[10]))

            for token in temp_list:
                temp = copy.deepcopy(target_neg)
                temp[training_pair[9]] = token
                target_list.append(temp)
                label.append(0)

            length_list = [training_pair[3]] * len_part_1 + [training_pair[8]] * (len(target_list) - len_part_1)

            assert len(target_list) == len(length_list)

            eq_output = seq_encoder.batch_forward(torch.stack(target_list).transpose(0, 1).cuda(), length_list)
            problem_output = problem_output.repeat(len(target_list), 1)
            #out = torch.bmm(problem_output.squeeze().unsqueeze(1), eq_output.squeeze().unsqueeze(-1))
            out = torch.sigmoid(torch.bmm(problem_output.squeeze().unsqueeze(1), eq_output.squeeze().unsqueeze(-1)))
            #out = torch.where(torch.isnan(out), torch.full_like(out, 0), out)
            #out = torch.softmax(torch.bmm(problem_output.unsqueeze(1), eq_output.unsqueeze(-1)), dim = 0).squeeze()
            #out = f.cosine_similarity(problem_output, eq_output)
            #for idx in range(out.size(0)):
            #    if idx != 0:
            #        loss_cl += max(0, out[idx] - out[0] + 0.5)
            if eq_output.isnan().any() or out.isnan().any():
                loss_cl += 0
            else:
                loss_cl += f.binary_cross_entropy(out.squeeze(), torch.FloatTensor(label).cuda())
            #loss_cl += -torch.log(out[0] / torch.sum(out, dim = 0))
            #if torch.max(out, 0)[1] == 0:
            #    crt_cnt_grad += 1

        #acc = crt_cnt_grad/total_cnt_grad
        loss_cl = loss_cl / total_cnt_grad
        #if random.random() < 0.01:
        #    print('acc', acc)
        loss_cl.backward()
        seq_encoder_optimizer.step()
    ...
    #score = torch.max(1-out, torch.zeros(out.size()).cuda())
    #loss_cl =  score * (score > 0.5)
    #try:
    ...
    if epoch > 10:
    #if epoch > 10:
        eq_output = seq_encoder.batch_forward(copy_target.cuda(), target_length)
        out = torch.sigmoid(torch.bmm(problem_output.unsqueeze(1), eq_output.unsqueeze(-1)))
        if eq_output.isnan().any() or out.isnan().any():
            loss_cl = 0
        #out = torch.where(torch.isnan(out), torch.full_like(out, 0), out)
        else:
            loss_cl = f.binary_cross_entropy(out.squeeze(), torch.ones(out.squeeze().size()).cuda())
        loss += 0.001 * loss_cl
    # loss = loss_0 + loss_1
    '''
    loss.backward()
...
def evaluate_tree_bert(input_batch, input_length, generate_nums, encoder, predict, generate, merge, output_lang, num_pos,
                  beam_size=5, english=False, max_length=MAX_OUTPUT_LENGTH):
    ...
    encoder_outputs, problem_output = encoder.evaluate(input_var, [input_length])
...
def pad_input(seq, seq_len, max_length):
    seq = copy.deepcopy(seq)
    seq += [0 for _ in range(max_length - seq_len)]
    return seq

def train_dis(encoder, first_dis, second_dis, encoder_optimizer, first_dis_optimizer, second_dis_optimizer, first_pos, first_neg, second_pos, second_neg):
    batch_size = len(first_pos)
    lengths_1 = []
    lengths_2 = []
    lengths_3 = []
    lengths_4 = []
    ...
    first_pos = sorted(first_pos, key=lambda tp: tp[1], reverse=True)
    first_neg = sorted(first_neg, key=lambda tp: tp[1], reverse=True)
    second_pos = sorted(second_pos, key=lambda tp: tp[1], reverse=True)
    second_neg = sorted(second_neg, key=lambda tp: tp[1], reverse=True)

    for i in range(batch_size):
        batches_1.append(pad_input(first_pos[i][0], first_pos[i][1], first_pos[0][1]))
        batches_2.append(pad_input(first_neg[i][0], first_neg[i][1], first_neg[0][1]))
        batches_3.append(pad_input(second_pos[i][0], second_pos[i][1], second_pos[0][1]))
        batches_4.append(pad_input(second_neg[i][0], second_neg[i][1], second_neg[0][1]))
        lengths_1.append(first_pos[i][1])
        lengths_2.append(first_neg[i][1])
        lengths_3.append(second_pos[i][1])
        lengths_4.append(second_neg[i][1])
    
    first_pos = torch.LongTensor(batches_1).transpose(0, 1)
    first_neg = torch.LongTensor(batches_2).transpose(0, 1)
    second_pos = torch.LongTensor(batches_3).transpose(0, 1)
    second_neg = torch.LongTensor(batches_4).transpose(0, 1)
    if USE_CUDA:
        first_pos = first_pos.cuda()
        first_neg = first_neg.cuda()
        second_pos = second_pos.cuda()
        second_neg = second_neg.cuda()
    encoder_optimizer_2 = torch.optim.Adam(encoder.parameters(), lr=1e-4, weight_decay=1e-5)
    ...
    first_dis_optimizer.zero_grad()
    second_dis_optimizer.zero_grad()

    _, problem_output_1 = encoder(first_pos, lengths_1)
    _, problem_output_2 = encoder(first_neg, lengths_2)
    _, problem_output_3 = encoder(second_pos, lengths_3)
    _, problem_output_4 = encoder(second_neg, lengths_4)
    problem_shuffled_1 = problem_output_1[torch.randperm(problem_output_1.size()[0])]
    problem_shuffled_3 = problem_output_3[torch.randperm(problem_output_3.size()[0])]

    pos_logits_1 = first_dis(problem_output_1, problem_shuffled_1)
    neg_logits_1 = first_dis(problem_output_1, problem_output_2)
    pos_logits_2 = first_dis(problem_output_3, problem_shuffled_3)
    neg_logits_2 = first_dis(problem_output_3, problem_output_4)

    loss = f.binary_cross_entropy_with_logits(pos_logits_1, torch.ones(pos_logits_1.size()).float().cuda()) +\
            f.binary_cross_entropy_with_logits(neg_logits_1, torch.zeros(neg_logits_1.size()).float().cuda()) +\
            f.binary_cross_entropy_with_logits(pos_logits_2, torch.ones(pos_logits_2.size()).float().cuda()) +\
            f.binary_cross_entropy_with_logits(neg_logits_2, torch.zeros(neg_logits_2.size()).float().cuda())

    loss.backward()
    encoder_optimizer_2.step()
    first_dis_optimizer.step()
    second_dis_optimizer.step()

    return loss.item()


def train_tree_together(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               encoder, predict, generate, merge, first_dis, second_dis, encoder_optimizer, predict_optimizer, generate_optimizer,
               merge_optimizer, first_dis_optimizer, second_dis_optimizer, first_pos, first_neg, second_pos, second_neg, output_lang, num_pos, epoch, english=False):
    # sequence mask for attention
    seq_mask = []
    op_list = [0,1,2,3]
    max_len = max(input_length)
    ...
    loss_dis = f.binary_cross_entropy_with_logits(pos_logits_1, torch.ones(pos_logits_1.size()).float().cuda()) +\
            f.binary_cross_entropy_with_logits(neg_logits_1, torch.zeros(neg_logits_1.size()).float().cuda()) +\
            f.binary_cross_entropy_with_logits(pos_logits_2, torch.ones(pos_logits_2.size()).float().cuda()) +\
            f.binary_cross_entropy_with_logits(neg_logits_2, torch.zeros(neg_logits_2.size()).float().cuda())
    ...
    # op_target = target < num_start
    # loss_0 = masked_cross_entropy_without_logit(all_leafs, op_target.long(), target_length)
    loss = masked_cross_entropy(all_node_outputs, target, target_length) + 0.01 * loss_dis
    '''
    if epoch > 10:
    #if epoch > 10:
        eq_output = seq_encoder.batch_forward(copy_target.cuda(), target_length)
    ...
    torch.nn.utils.clip_grad_norm_(generate.parameters(), 5)
    torch.nn.utils.clip_grad_norm_(merge.parameters(), 5)
    torch.nn.utils.clip_grad_norm_(first_dis.parameters(), 5)
    torch.nn.utils.clip_grad_norm_(second_dis.parameters(), 5)
...
def train_tree_all_together(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               encoder, predict, generate, merge, first_dis, second_dis, seq_encoder, encoder_optimizer, predict_optimizer, generate_optimizer,
               merge_optimizer, first_dis_optimizer, second_dis_optimizer, seq_encoder_optimizer, first_pos, first_neg, second_pos, second_neg, output_lang, num_pos, epoch, english=False):
    # sequence mask for attention
    seq_mask = []
    op_list = [0,1,2,3]
    max_len = max(input_length)
    ...
    if True:
        ...
        total_cnt_grad = 0
        crt_cnt_grad = 0
        n0_pos = output_lang.word2index['N0']
        for training_pair in training_pairs:
            max_target_len = max(training_pair[3], training_pair[8])
            total_cnt_grad += 1
            #target_ori = training_pair[2][:training_pair[3]].cuda().long()
            ...
            if training_pair[5] < num_start:
                ...
                temp_list = [n0_pos + i for i in range(training_pair[6])]
            #if training_pair[5] != 4 and training_pair[5] != 5 and training_pair[5] != 6 and training_pair[5] != 22:
            if int(training_pair[5]) in temp_list:
                temp_list.remove(int(training_pair[5]))
            try:
                for token in temp_list:
                    temp = copy.deepcopy(target_ori)
            ...
            if training_pair[10] < num_start:
                ...
                temp_list = [n0_pos + i for i in range(training_pair[6])]
            #if training_pair[10] != 5 and training_pair[10] != 6 and training_pair[10] != 4 and training_pair[10] != 22:
            if int(training_pair[10]) in temp_list:
                temp_list.remove(int(training_pair[10]))
    ...
    try:
        loss = masked_cross_entropy(all_node_outputs, target, target_length) + 0.001 * loss_cl + 0.01 * loss_dis
    except:
    ...
    #return loss.item(), 1, loss_cl.item()
    return loss.item(), loss_dis.item(), loss_cl.item()



def train_tree_all_together_bert(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               encoder, predict, generate, merge, first_dis, second_dis, seq_encoder, encoder_optimizer, predict_optimizer, generate_optimizer,
               merge_optimizer, first_dis_optimizer, second_dis_optimizer, seq_encoder_optimizer, first_pos, first_neg, second_pos, second_neg, output_lang, num_pos, epoch, english=False):
    # sequence mask for attention
    seq_mask = []
        ...
        if True:
            for ii in range(len(input_length)):
                if True:
                    encoder_optimizer.zero_grad()
                    seq_encoder_optimizer.zero_grad()
                    _, problem_output = encoder.evaluate(input_var[ii], torch.tensor([input_length[ii]]))
                    eq_output = seq_encoder.cal_grad(target[:,ii].unsqueeze(-1).cuda(), target_length[ii])
                    #out = torch.bmm(problem_output.unsqueeze(1), eq_output.unsqueeze(-1))
                    #out = f.cosine_similarity(problem_output, eq_output)
                    out = torch.sigmoid(torch.bmm(problem_output.unsqueeze(1), eq_output.unsqueeze(-1)))
            ...
            for training_pair in training_pairs:
                ...
                label = [1]
                target_list = [target_ori]
                _,problem_output = encoder.evaluate(training_pair[0], torch.tensor([training_pair[1]]))
    ...
    try:
        if True:
            ...
            for i in range(batch_size_1):
                batches_1.append(first_pos[i][0])
                batches_2.append(first_neg[i][0])
                batches_3.append(second_pos[i][0])
                batches_4.append(second_neg[i][0])
                lengths_1.append(first_pos[i][1])
                lengths_2.append(first_neg[i][1])
                lengths_3.append(second_pos[i][1])
                lengths_4.append(second_neg[i][1])
            ...
            _, problem_output_1 = encoder(batches_1, lengths_1)
            _, problem_output_2 = encoder(batches_2, lengths_2)
            _, problem_output_3 = encoder(batches_3, lengths_3)
            _, problem_output_4 = encoder(batches_4, lengths_4)
            problem_shuffled_1 = problem_output_1[torch.randperm(problem_output_1.size()[0])]
            problem_shuffled_3 = problem_output_3[torch.randperm(problem_output_3.size()[0])]
    ...
    #return loss.item(), 1, 1
    if loss_cl != 0:
        return_loss_cl = loss_cl.item()
    else:
        return_loss_cl = 0
    if loss_dis != 0:
        return_loss_dis = loss_dis.item()
    else:
        return_loss_dis = 0
    return loss.item(), return_loss_dis, return_loss_cl


def train_tree_vanilla_bert(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               encoder, predict, generate, merge, first_dis, second_dis, seq_encoder, encoder_optimizer, predict_optimizer, generate_optimizer,
               merge_optimizer, first_dis_optimizer, second_dis_optimizer, seq_encoder_optimizer, first_pos, first_neg, second_pos, second_neg, output_lang, num_pos, epoch, english=False):
    # sequence mask for attention
    seq_mask = []
...
def train_tree_vanilla(input_batch, input_length, target_batch, target_length, nums_stack_batch, num_size_batch, generate_nums,
               encoder, predict, generate, merge, first_dis, second_dis, seq_encoder, encoder_optimizer, predict_optimizer, generate_optimizer,
               merge_optimizer, first_dis_optimizer, second_dis_optimizer, seq_encoder_optimizer, first_pos, first_neg, second_pos, second_neg, output_lang, num_pos, epoch, english=False):
    # sequence mask for attention
    seq_mask = []
