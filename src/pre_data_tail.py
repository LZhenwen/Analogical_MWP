...
def load_mathqa_data(filename, problem_list):  # load the json data to list(dict()) for MATH 23K
    print("Reading lines...")
    f = open(filename, encoding="utf-8")
    js = ""
    data = []
    for i, s in enumerate(f):
        ...
        if i % 5 == 0:  # every 5 line is a json
            data_d = json.loads(js)
            text = data_d['original_text']
            if text in problem_list:
                data.append(data_d)
            js = ""
...
def transfer_num_bert(data):  # transfer num into "NUM"
    print("Transfer numbers...")
    pattern = re.compile("\d*\(\d+/\d+\)\d*|\d+\.\d+%?|\d+%?")
    pairs = []
    generate_nums = []
...
def transfer_num_mathqa(data):  # transfer num into "NUM"
    print("Transfer numbers...")
    pattern = re.compile("\d*\(\d+/\d+\)\d*|\d+\.\d+%?|\d+%?")
    pairs = []
    generate_nums = []
...
def transfer_num_mathqa_for_SVAMP(data):  # transfer num into "NUM"
    print("Transfer numbers...")
    pattern = re.compile("\d*\(\d+/\d+\)\d*|\d+\.\d+%?|\d+%?")
    pairs = []
    generate_nums = []
    ...
    for idx, d in enumerate(data):
        ...
        if len(from_infix_to_prefix(out_seq)) == 3 or len(from_infix_to_prefix(out_seq)) == 5:
            if '=' not in input_seq and '+' not in input_seq and '-' not in input_seq and '*' not in input_seq and '**' not in input_seq:
                pairs.append((input_seq, out_seq, nums, num_pos, idx))
...
def prepare_bert_data(pairs_trained, pairs_tested, trim_min_count, generate_nums, copy_nums, tree=False):
    input_lang = Lang()
    output_lang = Lang()
    train_pairs = []
    test_pairs = []
    ...
    for pair in pairs_trained:
        ...
        #print([tokenizer.convert_ids_to_tokens(ii) for ii in inputs['input_ids']])
        num_stack = []
        for word in pair[1]:
            temp_num = []
            flag_not = True
        ...
        # train_pairs.append((input_cell, len(input_cell), output_cell, len(output_cell),
        #                     pair[2], pair[3], num_stack, pair[4]))
        train_pairs.append((inputs, inputs['input_ids'].squeeze().size(0), output_cell, len(output_cell),
    ...
    for pair in pairs_tested:
        ...
        # train_pairs.append((input_cell, len(input_cell), output_cell, len(output_cell),
        #                     pair[2], pair[3], num_stack, pair[4]))
        test_pairs.append((inputs, inputs['input_ids'].squeeze().size(0), output_cell, len(output_cell),
                           pair[2], num_pos, num_stack))
    print('Number of testind data %d' % (len(test_pairs)))
    return input_lang, output_lang, train_pairs, test_pairs


def prepare_bert_data_type(pairs_trained, pairs_tested, trim_min_count, generate_nums, copy_nums, tree=False):
    input_lang = Lang()
    output_lang = Lang()
    train_pairs = []
    test_pairs = []
    ...
    for pair in pairs_tested:
        ...
        #                     pair[2], pair[3], num_stack, pair[4]))
        test_pairs.append((inputs, inputs['input_ids'].squeeze().size(0), output_cell, len(output_cell),
                           pair[2], num_pos, num_stack, pair[-1]))
    print('Number of testind data %d' % (len(test_pairs)))
    return input_lang, output_lang, train_pairs, test_pairs

def prepare_en_bert_data(pairs_trained, pairs_tested, trim_min_count, generate_nums, copy_nums, tree=False):
    input_lang = Lang()
    output_lang = Lang()
    train_pairs = []
    test_pairs = []
    ...
    for pair in pairs_trained:
        ...
        num_pos = []
        for idx,i in enumerate(inputs['input_ids'].squeeze()):
            if tokenizer.convert_ids_to_tokens(int(i)) == 'nu':
                num_pos.append(idx)
...
def prepare_train_batch_bert(pairs_to_batch, batch_size):
    pairs = copy.deepcopy(pairs_to_batch)
    random.shuffle(pairs)  # shuffle the pairs
    pos = 0
    input_lengths = []
