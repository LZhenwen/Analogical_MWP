...
class EncoderRNN(nn.Module):
    ...
    def forward(self, input_seqs, input_lengths, hidden=None):
        ...
        #problem_output = outputs[-1, :, :self.hidden_size] + outputs[0, :, self.hidden_size:]
        # S x B x H
        return torch.clamp(out.mean(0), -5, 5)

    def batch_forward(self, input_seqs, input_lengths, token = '', hidden=None):
        # Note: we run this all at once (over multiple batches of multiple sequences)
        embedded = self.embedding(input_seqs)  # S x B x E
        self.embedded = embedded
        embedded = self.em_dropout(embedded)
        ...
        pade_outputs, pade_hidden = self.gru(packed, hidden)
        pade_outputs, _ = torch.nn.utils.rnn.pad_packed_sequence(pade_outputs)
        #outputs, output_lengths = torch.nn.utils.rnn.pad_packed_sequence(outputs)  # unpack (back to padded)
        out = pade_outputs[:, :, :self.hidden_size] + pade_outputs[:, :, self.hidden_size:]  # Sum bidirectional outputs

        #problem_output = outputs[-1, :, :self.hidden_size] + outputs[0, :, self.hidden_size:]
        # S x B x H
        out = out.sum(0)/((out!=0).sum(0))
        out = torch.clamp(out, -5, 5)
        return out
        
    def cal_grad(self, input_seqs, input_lengths, hidden=None):
        # Note: we run this all at once (over multiple batches of multiple sequences)
        embedded = self.embedding(input_seqs)  # S x B x E
        self.embedded = embedded
...
class Discriminator(nn.Module):
    ...
    def forward(self, hidden_0, hidden_1):
        concated = torch.cat([hidden_0, hidden_1], dim = -1)
        out = self.l2(self.relu(self.l1(concated)))

        return out

class Discriminator_bmm(nn.Module):
    def __init__(self, hidden_size):
        super(Discriminator_bmm, self).__init__()
        self.hidden_size = hidden_size
    ...
    def forward(self, hidden_0, hidden_1):
        ...
        hidden = self.l1(hidden_0)
        
        out = torch.bmm(hidden.unsqueeze(1), hidden_1.unsqueeze(-1))

        return out

class Discriminator_bmm2(nn.Module):
    def __init__(self, hidden_size):
        super(Discriminator_bmm2, self).__init__()
        self.hidden_size = hidden_size
        self.l1 = nn.Linear(hidden_size, hidden_size)
    ...
    def forward(self, hidden_0, hidden_1):
        ...
        hidden_0 = self.l2(self.l1(hidden_0))
        hidden_1 = self.l1(hidden_1)
...
class Bert_Encoder(nn.Module):
    def __init__(self, hidden_size):
        ...
        self.hidden_size = hidden_size
        #self.bert_rnn = BertModel.from_pretrained("/home/lianz0a/GTS_contrastive/models/all_epoch_30")
        #self.bert_rnn = BertModel.from_pretrained("/ibex/scratch/lianz0a/bert2tree-master/models/all_epoch_30")

    def forward(self, bert_encoding, length):
        # Note: we run this all at once (over multiple batches of multiple sequences)
    ...
    def evaluate(self, bert_encoding, length):
...
class Bert_Encoder_en(nn.Module):
    def __init__(self, hidden_size):
        super(Bert_Encoder_en, self).__init__()

        
        self.hidden_size = hidden_size
        #configuration = AutoConfig.from_pretrained('bert-base-uncased')
        #self.bert_rnn = BertModel.from_pretrained(pretrained_model_name_or_path = 'bert-base-uncased', config = configuration)
        #self.bert_rnn = BertModel.from_pretrained(pretrained_model_name_or_path = 'bert-base-uncased')
        


        self.bert_rnn = BertModel.from_pretrained("/home/zliang6/Documents/GTS_contrastive/models/mwpbert_en")
        #self.bert_rnn = BertModel.from_pretrained("/ibex/scratch/lianz0a/bert2tree-master/models/all_epoch_30")
