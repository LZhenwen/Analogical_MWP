...
pairs_train, generate_nums, copy_nums = transfer_num_mathqa(data)
data = load_mathqa_data("data/MathQA_test.json")
pairs_test, _, _ = transfer_num_mathqa(data)


pairs_trained = []
for p in pairs_train:
    pairs_trained.append((p[0], from_infix_to_prefix(p[1]), p[2], p[3], p[4]))
pairs_tested = []
for p in pairs_test:
    pairs_tested.append((p[0], from_infix_to_prefix(p[1]), p[2], p[3], p[4]))
...
for epoch in range(n_epochs):
    ...
    for idx in range(len(input_lengths)):
        ...
        first_pos = random.sample(d[first_random], min(batch_size,len(d[first_random])))
        temp = []
        for sub_list in d:
            if sub_list != first_random:
                temp = temp + d[sub_list]
        first_neg = random.sample(temp, min(batch_size,len(d[first_random])))

        second_random = random.choice(list(dd.keys()))
        second_pos = random.sample(dd[second_random], min(batch_size,len(dd[second_random])))
        temp = []
        for sub_list in dd:
            if sub_list != second_random:
                temp = temp + dd[sub_list]
        second_neg = random.sample(temp, min(batch_size,len(dd[second_random])))
    ...
    if epoch % 5 == 0 or epoch > n_epochs - 10:
        ...
        if epoch > n_epochs - 10:
            torch.save(encoder.state_dict(), "models/encoder_all_mathqa"+str(epoch))
            torch.save(predict.state_dict(), "models/predict_all_mathqa"+str(epoch))
            torch.save(generate.state_dict(), "models/generate_all_mathqa"+str(epoch))
            torch.save(merge.state_dict(), "models/merge_all_mathqa"+str(epoch))
